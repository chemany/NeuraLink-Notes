...
export default function NotebookDetailLayout({
  ...
  // 如果没有 notebookId (例如 /notebook/ 页面本身，而不是 /notebook/[id])
  // 处理 /notebook 根路径的情况
  if (pathname === '/notebook' || pathname === '/notebook/') {
    return (
      <div className="flex h-screen w-screen items-center justify-center">
        <p>请从主页选择一个笔记本，或 <a href="/" className="text-blue-500 hover:underline">返回首页</a> 创建新的笔记本。</p>
        {/* children 可能会渲染 /notebook/page.tsx 的内容，如果该文件存在的话 */}
        {/* {children} */}
      </div>
    );
  }
  
  // 对于其他 /notebook 下的未知子路径，或者 notebookId 仍在确定中但未到加载状态
  // 可以返回一个通用的加载状态或错误提示，或者直接渲染 children （如果适用）
  // 为避免在 notebookId 尚未从 params 中解析出来时就渲染错误，这里的逻辑需要小心
  // 上面的 isAuthLoading 和 isLoadingNotebooks 应该覆盖了大部分初始加载情况
  // 如果 params 为空 (例如在非动态路由的 /notebook/someotherpage)，notebookId 会是 null
  if (!notebookId && !isAuthLoading) { // 如果 notebookId 最终为 null 且不是因为加载
     return (
      <div className="flex h-screen w-screen items-center justify-center">
